...
- Phase 4: Technical accuracy pass and cross-check against Proxmox 8.x behavior.
...
- [ ] Run `make build-ja` and confirm the chapter renders correctly (once the build tooling is available in this repo).
...
- [ ] `make build-ja` completes successfully, and the resulting PDF includes the chapter with correct page breaks.
- [ ] A technical reviewer (Ota) has skimmed the chapter and confirmed that installation steps match Proxmox 8.x behavior.
...
- Build tooling (`make build-ja`) is not yet present in this repo; validation steps relying on it are recorded but cannot be executed in this pass.
...
- Decision: Proceed with a text-complete draft and placeholder diagram/paths for screenshots, leaving the actual screenshot capture and PDF layout validation to a later pass when a reference environment and build tooling are available.
