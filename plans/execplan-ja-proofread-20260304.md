...
- Constraints: 「4. Constraints and assumptions」を参照
- Related: `.agent/PLANS.md`, `STYLEGUIDE.md`, `POLICY.md`

## 2. Purpose and big picture

- 全章の校正・リライトにより、誤字脱字/表記ゆれ/論理の粗を減らし、読者が迷わず読み進められる状態にする
- `manuscript/ja/` を正本として修正し、派生物（`docs/` / `build/ja/book.md`）は同期して整合を取る
- Book QA 相当のチェックを通し、公開物の品質劣化を抑止する

## 3. Context and orientation

- 正本: `manuscript/ja/`（章ファイル）
- 派生物:
  - `docs/`（GitHub Pages 用）
  - `build/ja/book.md`（結合原稿）
- 同期/検証の基本手順: `make build-ja` → `make sync-docs-ja` → `make check-ja`
...
- [x] `make build-ja` → `make sync-docs-ja` → `make check-ja` を実行（コミット後に再実行して差分ゼロを確認）
...
- [x] ブランチ `qa/proofread-20260304` でコミット→ push → PR 作成

## 7. Validation and acceptance

- [x] `make check-ja` が成功する（`build-ja` + `sync-docs-ja` 後に差分なし）
- [x] Book QA 相当のチェックが成功する
- [x] PR には変更ファイル、主要改善点、断定更新しなかった箇所と理由、手動確認点、検証結果を記載する

## 8. Progress

- [2026-03-04] Plan created.
- [2026-03-04] `manuscript/ja/**` を校正（表記ゆれ/文体/安全注意の明確化）。`make build-ja` / `make sync-docs-ja` / `make check-ja` 実行（成功、差分なし）。book-formatter（unicode/textlint/links/layout-risk/structure）実行。
