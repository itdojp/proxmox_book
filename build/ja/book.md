...
- [ ] ラボ構成を決める（パターン A/パターン B）: ノード名、管理 IP アドレス、（可能なら）VLAN の割り当てをメモに書く
...
補足: カーネルや systemd/udev の更新に伴い、ネットワークインターフェース名が変わることがあります。
複数 NIC 構成や本番想定の構成では、更新時の影響を見積もったうえで、必要に応じてインターフェース名の固定（永続化）も検討ポイントになります。
...
この 3 点が揃っていないと、Proxmox VE 自体が優れていても「運用事故が発生しやすい状態」になりがちです。
