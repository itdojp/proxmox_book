...
スクリーンショット（TODO）:
- 本書では `images/part1/ch3/` にインストール関連のスクリーンショットを配置し、画面遷移を追えるようにします。
- 取得対象の一覧は Issue #2 を参照してください（インストールメニュー、ライセンス、ディスク、ロケール、ネットワーク、完了画面、初回ログイン等）。
...
スクリーンショット（TODO）:
- 取得対象の一覧は Issue #2 を参照してください（バックアップジョブ一覧、作成ウィザード、タスクログ、リストアダイアログ等）。
