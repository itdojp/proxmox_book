...
Proxmox VE 9.1 の主な更新点として、公式リリース情報（2026-05-23（Asia/Tokyo）確認）では、
OCI イメージからの LXC コンテナ作成、vTPM 状態の qcow2 形式での扱い、ネスト仮想化の細かな制御、
SDN スタックの状態表示強化が示されています。
本書ではこれらをすべて詳細手順として扱うのではなく、
関連章で「採用するか」「今回は範囲外にするか」を判断するための確認観点として扱います。
...
「小〜中規模のオンプレミス環境で、どのような場面で Proxmox VE が有力な選択肢となり得るか」
...
- [ ] 本書を「どこまでやるか」を決める（単一ノードまで / 3 ノードクラスタまで / 特定パートだけ、など）
- [ ] `manuscript/ja/SUMMARY.md` を開き、読む順番（章）とゴールを書き出す
- [ ] 自分のラボ環境の前提（例: `pve1` の IP アドレス、DNS/NTP、バックアップ退避先）をメモに固定する
...
次は Part 0 の「執筆環境・検証環境の準備」で、ラボ構成の前提を整理します。
# 執筆環境・検証環境の準備
...
- 分かること:
  ...
  - 単一ノード/3 ノードクラスタ（ネスト可）のラボ構成パターン
  - 検証環境で事前に決めておくべき項目（命名、IP アドレス、DNS/NTP など）
- 分からないこと（後続章で扱います）:
  - インストーラの画面操作（第3章）
...
ただし、カーネル（例: 6.x 系）や UI はマイナー更新で変わることがあります。
...
- 管理用 IP アドレス: 例 `192.168.10.11`（固定 IP アドレスを推奨）
...
本書で想定する 3 ノードクラスタのラボは、ネスト構成でも進められるように設計しています（物理 3 台でも同様の考え方で適用できます）。
...
- [ ] ラボ構成を決める（パターン A/パターン B）: ノード名、管理用 IP アドレス、（可能なら）VLAN の割り当てをメモに書く
- [ ] 既に Proxmox VE を用意できている場合は、`pveversion -v` / `uname -r` の結果を記録する
- [ ] バックアップ退避先（別ディスク/NAS/別ホスト）を決め、保存先ストレージ名（例: `local` / `backup-nfs`）の候補を決める

## まとめ

- 本書で想定するラボの前提として、ハードウェア要件の目安と検証方法（ベアメタル/ネスト/クラウド）を整理しました。
- ラボ構成は「単一ノード（基本編）」と「3 ノードクラスタ（発展編）」の 2 パターンを想定し、学習範囲に応じて選べるようにしました。
- ストレージとバックアップは、I/O の集中や復旧の観点から、ディスク構成・保存先・データの扱いに注意して設計します。
...
この章では、Proxmox VE が「何をするための製品か」「どのような規模や用途に向いているか」を把握し、
...
そのため、小〜中規模の環境でも、過度に多くのミドルウェアを組み合わせることなく、仮想化基盤を構築できます。
...
- 組織内の知見・サポート体制  
  Proxmox VE に慣れたメンバーが少ない組織では、トラブルシュートや設計判断に時間がかかる場合があります。
...
## Proxmox VE 9.1 で特に確認する入口

Proxmox VE 9.1 の更新点は、既存の基本構成を置き換えるものではなく、
VM/コンテナ、ストレージ、ネットワークの各レイヤーに追加の選択肢を与えるものとして捉えると整理しやすくなります。
本書の範囲では、次のように読み替えてください。

| 更新点 | 関係するレイヤー | 本書での扱い |
| --- | --- | --- |
| OCI イメージからの LXC コンテナ作成 | ゲスト層（コンテナ） | LXC はホストカーネルを共有する前提を押さえ、イメージの由来・更新責任・権限境界を確認する |
| vTPM 状態の qcow2 形式での扱い | VM / ストレージ | vTPM を使う Windows 系・セキュリティ要件付き VM では、スナップショット/復元テストの対象に含める |
| ネスト仮想化の細かな制御 | VM / CPU | ネスト検証や VBS など、必要な VM だけで有効化し、ホスト CPU を安易に全面公開しない |
| SDN 状態表示の強化 | ネットワーク | 本書は Linux ブリッジ/VLAN を基本に扱い、SDN/EVPN は状態確認の入口に留める |
...
トラブル時は、症状から「どのレイヤーの問題か」を仮置きし、入口（Web UI/CLI/ログ）を決めると切り分けが進めやすくなります。
以下は最小限の目安です（実際の表示名やログの場所は、バージョンや構成で差異があり得ます）。

| 例の症状 | まず見る（入口） | 次に見る（深掘り） | コマンド例（必要に応じて） |
| --- | --- | --- | --- |
| Web UI に到達できない | ネットワーク疎通、URL/ポート（`:8006`） | ノード上の Web UI 関連サービスとログ | `ss -lntp | grep ':8006'` / `systemctl status pveproxy --no-pager` / `journalctl -u pveproxy -n 50 --no-pager` |
| Web UI 操作が失敗する（Tasks が失敗） | `Tasks` のエラーメッセージ | 対象サービスのログ（ホスト OS） | `journalctl -u pvedaemon -n 50 --no-pager`（例） |
| VM が起動しない / すぐ止まる | 対象 VM の `Tasks` | ストレージ状態、VM の状態 | `pvesm status` / `qm status <VMID>`（例） |
| クラスタが不安定 / クォーラムが落ちる | `Datacenter → Cluster` / `pvecm status` | corosync / pve-cluster のログ | `journalctl -u corosync -u pve-cluster -n 50 --no-pager` |
...
- 「Web UI の操作は “見た目だけ” で、何が変わったか追えない」  
  実際には、設定はホスト側の設定ファイルや内部の定義として保持されます。困ったときに「どの層（レイヤー）で何が起きているか」を切り分けやすくするため、本章の見取り図が役立ちます。
...
- ネットワーク接続と IP アドレス割り当て方針（固定 IP アドレスを推奨）
...
補足: Linux では NIC の構成やドライバ、カーネルや systemd/udev の更新により、ネットワークインターフェース名が変わることがあります。
複数 NIC 構成や本番想定の構成では、運用上の影響を見積もったうえで、必要に応じてインターフェース名を固定する（例: systemd の `.link` ルール）ことも検討ポイントになります。
...
- ノード名（ホスト名）: 例 `pve1`
- 管理用 IP アドレス: 例 `192.168.10.11/24`
...
補足:
- URL は `https://<管理 IP アドレス>:8006/` の形式になることが一般的ですが、まずは **コンソールに表示された URL** をそのまま使ってください。
...
- `ip -br a` / `ip r`: 管理用 IP アドレスとデフォルトゲートウェイが想定どおりである
...
- ブラウザで `https://<管理 IP アドレス>:8006/` を開けるか（ポート番号の付け忘れに注意）
...
- [ ] Web UI にログインできることを確認する（`https://<管理 IP アドレス>:8006/`、ユーザーは `root@pam`）
...
## セキュリティ系ゲストと vTPM / ネスト仮想化の注意

Proxmox VE 9.1 では、vTPM 状態を qcow2 形式で扱う改善や、ネスト仮想化を細かく制御するための選択肢が追加されています。
本書の最小ハンズオンでは必須ではありませんが、次のような VM を作る場合は、作成時点で運用前提を確認してください。

| 場面 | 確認すること |
| --- | --- |
| Windows やセキュリティ要件付き VM で vTPM を使う | ストレージ形式、スナップショット可否、復元テストの対象に vTPM 状態を含めるか |
| BitLocker など鍵管理と結びつく機能を使う | バックアップ/復元時に鍵や回復キーへアクセスできるか |
| ネスト仮想化や VBS を検証する | 必要な VM だけで有効化し、性能・セキュリティ・サポート範囲をラボで確認する |

注意: スナップショットや vTPM 対応はバックアップの代替ではありません。
本番に近い VM では、少なくとも 1 回は隔離された場所で復元し、起動・ログイン・アプリ疎通まで確認します。
...
| パターン | 構成 | 向いている場面 | 注意点 |
| --- | --- | --- | --- |
| A: 1 ブリッジ（最小） | `vmbr0` 1 つに管理/VM を集約 | まず動かす、単一ノード | 影響範囲が広い（誤設定すると全体に影響しやすい） |
| B: VLAN で分離 | `vmbr0` + VLAN（10=管理、20=VM…） | 物理 NIC が少ないが分離したい | スイッチ側 VLAN 設定が前提 |
| C: 物理分離（余裕がある場合） | 管理用 NIC と VM 用 NIC を分ける | 本番に近い検証 | 物理ポート/スイッチ側も含めた設計が必要 |
...
- 管理用 IP アドレスが想定したインターフェースに付いており、状態が `UP` である
...
管理用 IP アドレスをブリッジ（`vmbr0`）側に持たせる、最小構成の例です。
...
- [ ] ノードで `ip -br a` / `ip r` を実行し、管理用 IP アドレスとデフォルトゲートウェイが想定どおりか確認する
...
注意: クォーラムを失った状態では、クラスタを危険な状態にしないために一部の操作が制限されることがあります。まずは `pvecm status` の `Quorate: Yes/No` を確認し、状況を把握してください。
...
- 作業前にクォーラムを確認する（例: `pvecm status` が `Quorate: Yes`）
...
ラボ環境では、まずは手動実行や低頻度のスケジュールで動作を確認するところから始めるとよいでしょう。
...
トラブル時にいきなり深掘りすると手順を見失いがちです。まずは次の順で “入口” を確認します。
...
更新作業の前には、手元のノード情報だけでなく、公式のリリース情報とドキュメントを確認します。
特に 9.1 系では OCI 由来の LXC、vTPM、ネスト仮想化、SDN 表示など、利用有無によって確認観点が変わる機能があります。

| 確認対象 | 見る理由 | 入口 |
| --- | --- | --- |
| リリースノート / Roadmap | 既知の問題、破壊的変更、9.1 固有の更新点を確認する | `https://pve.proxmox.com/wiki/Roadmap` |
| 公式ドキュメント | 手順名や CLI の詳細を、本文ではなく一次情報で確認する | `https://pve.proxmox.com/pve-docs/` |
| 対象ノードの状態 | 実際に動いている版・カーネル・リポジトリを確認する | `pveversion -v` / `uname -r` / `apt update` |
| 採用機能の一覧 | vTPM、ネスト仮想化、SDN、LXC/OCI の利用有無を明示する | 運用チェックリストまたは変更申請 |
