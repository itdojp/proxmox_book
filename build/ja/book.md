...
補足: Linux では NIC の構成やドライバの変化により、ネットワークインターフェース名が変わることがあります。
複数 NIC 構成や本番想定の構成では、運用上インターフェース名を固定する（例: systemd の `.link` ルール）ことも検討ポイントになります。
...
- ブラウザで `https://<IPアドレス>:8006/` を開けるか（ポート番号の付け忘れに注意）
...
- [ ] Web UI にログインできることを確認する（`https://<管理用IPアドレス>:8006/`、ユーザーは `root@pam`）
...
この 3 点が揃っていないと、Proxmox VE 自体が優れていても「事故につながりやすい運用」になりがちです。
