...
- ストレージ、ネットワーク、クラスタ設定の変更は仮想マシン停止や到達性低下につながるため、まずラボ環境で手順を確認してください。
- 本番環境では、変更前のバックアップ取得、メンテナンス時間の確保、関係者への周知を前提に進めてください。
- UI や文言は 9.x 系の範囲でも変わることがあるため、画面の位置だけで判断せず、操作対象の意味を確認してから適用してください。
